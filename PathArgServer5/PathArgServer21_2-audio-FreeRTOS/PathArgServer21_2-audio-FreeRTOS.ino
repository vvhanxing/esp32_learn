...
char* readDocument(fs::FS &fs, const char * path);
...
uint8_t frame_0[1024*20 ] PROGMEM ={0};





const uint8_t *gif[1] PROGMEM {frame_0};
...
void handleGIFUpload() {
  if (server.method() == HTTP_POST) {
    ...
    if (frame_index =="0"){
      Serial.println( 0);
      Serial.println(sizeof(encodedImage));
      writeFile(LittleFS, "/1.txt", (char*)encodedImage.c_str());
//      readtxtFile(LittleFS, "/hello.txt");
...
//        for (int i=0;i<pic_length;i++ ){
          ...
          encodedImageRUN =readDocument(LittleFS, "/1.txt");// readtxtFile(LittleFS, "/1.txt");
          int decoded_size = base64_decode((char*)gif[0] , encodedImageRUN, 1024*20);
...
File file = fs.open(path); // 打开文件以供读取
    if (!file) {
...
// 读取文件内容到缓冲区
    size_t bufferSize = file.size(); // 获取文件大小
    char* buffer = new char[bufferSize + 1]; // 分配足够的缓冲区，并加1用于字符串的结束符 '\0'
    if (!file.readBytes(buffer, bufferSize)) {
     
        delete[] buffer; // 释放缓冲区内存
        return NULL; // 读取失败，返回空指针
    }
    buffer[bufferSize] = '\0'; // 在字符串末尾添加结束符 '\0'

    file.close(); // 关闭文件
   
    return buffer; // 返回读取的字符串
