...
// 定义一个自定义的SPI类实例
SPIClass CustomSPI;
void WriteFile_sd(const char *path, const char *message)
{
  // 尝试打开文件以便写入
 ...
 * 如果无法打开文件，将打印错误信息。
 */
void ReadFile_sd(const char *path)
{
...
void handleImageUpload() {
  if (server.method() == HTTP_POST) {
    ...
    WriteFile_sd("/uploadpic.txt", (char*)encodedImage.c_str());
...
////////////////////////////////////
  ...
  // 写入测试文件"/test.txt"
  WriteFile_sd("/test2.txt", "myfirstmessage");
