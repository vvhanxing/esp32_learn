...
void runOS(){
            ...
            if (clickB()) {
                ...
                if (clickB_push_count==1000){//长按
                  ...
                  if (dirList[OSIndexX]=="Book"){
                      
                      pRanderPage = renderBookPage;
                      pRanderPage(OSIndexX);  
//                      listDir(SD,"/sys/Book/data",0);    
//                      for  (int i ;i<3;i++) Serial.println(fileList[i]);             
                    }
                  }
...
// String page[] = {"Video.","Music.","Game.","Image.","MSG.","Setting."};
  ...
  for (int i = 0;i<6;i++){
   ...
   for (int t=0;t<20;t++){
     ...
     String picTxt =ReadFile_txt((char*)(sysPath+APPpath+APPicon).c_str());
     base64::decode((char*) picTxt.c_str(), frame_0);}
...
int renderBookPage(int index){
  ...
  tft.setCursor(10, 5);
  tft.print("Markdown Viewer"); 
  const char* text = (char *)ReadFile_txt("/sys/Book/data/book1.txt").c_str();
  //const char* text = "# Header 1\n## Header 2\n*Italic text*\n**Bold text**\n- List item 1\n- List item 2\nNormal text.";
  ...
  while (*p) {
    if (*p == '#') {
      ...
      while (*p == ' ') p++;  // 跳过空格
      tft.setTextSize(FONT_HEADER - headerLevel + 1);
      tft.setCursor(x, y);
      while (*p && *p != '\n') {
        tft.print(*p++);
      }
